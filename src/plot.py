...
from src.write_path import FOLDER_FIG, FOLDER_VID
...
my_palette = [color_map(.18), color_map(.48), color_map(.69)]
...
AXES_FONTSIZE = 15
LABELS = {'y_beta': r'Fragmentation rate per unit of size $\beta$',
...
def plot_division_rate(sizes, beta, fig_dir=None):
    ...
    # Saving.
    if not isinstance(fig_dir, type(None)):
        path = write_fig_path(fig_dir, f'division_rate.pdf')
        plt.savefig(path, bbox_inches='tight')
    plt.show()

def plot_evo_lambda_estimates(times, features, lambda_estimates, name,
                              t_max=None, fig_dir=None):#, context=1):
    ...
    """
    # Print the lastest estimations and define main plotting PARAMETERS.
    print('\n Kappa ' + name + '\n ' + '-' * ( 6+ len(name)))
    ...
    if len(lambda_estimates) == 2: # Conservative case.
        ...
        LEGEND = [LABELS['leg_lbd_n'], LABELS['leg_lbd_g'],
                  LABELS['leg_lbd_t']]
        LINESTYLES = ['-', '--', '-.']
    ...
    for i in range(estimate_count):
        ...
        plt.plot(times[2:tmax_idx],
                 lambda_estimates[i][:len(times[2:tmax_idx])], LINESTYLES[i],
                 label=LEGEND[i], color=my_palette[i])
...
def plot_evo_log_n_sum(times, n_sum_evos, labels, fig_dir=None, name='',
                       fig_size=(6.4, 4.8), bbox_to_anchor=None):
    ...
    for i in range(len(n_sum_evos)):
        plt.plot(times[1:], np.log(n_sum_evos[i][1:]), LINESTYLES[i],
...
def plot_distribution_old(
        sizes, features, distribution, fig_name='', fig_dir=None, kappa=None,
        y_key_label='y_density', is_normalized=False,
        is_normalized_by_v=False):
    """ Plot the initial distribution (given by `distribution` at sizes `sizes`
    and features `features`) and save it if `fig_dir` is not None.
    ...
    for feature_idx in range(feature_count):
        plt.plot(sizes, dist[feature_idx], LINESTYLES[feature_idx],
...
def plot_distribution(
    ...
    if labels is None:
        labels = [write_feature_lengend(features[i], index=i + 1) for i in
                  range(feature_count)]
    ...
    for i in np.arange(plot_count)[::None]:
        plt.plot(sizes[:idx_max+1], dist[i][:idx_max+1], LINESTYLES[i],
                 color=COLORS[i], label=labels[i])
        # Fill the area under each line
        plt.fill_between(sizes[:idx_max+1], dist[i][:idx_max+1],
                         color=COLORS[i], alpha=0.2)

    if kappa is not None:
        ax = plt.gca()
        kappa_str = kappa if isinstance(kappa, str) else \
            write_pmatrix(kappa)
        anchored_text = matplotlib.offsetbox.AnchoredText(
            kappa_str, loc="upper left", frameon=False, pad=0.2)
        ax.add_artist(anchored_text)
...
def plot_evo_distribution_continuous(times, sizes, features, n_evo,
                                     x_max=None, name='', is_saved=False,
                                     fig_dir=FOLDER_VID, t_max=None,
                                     xtest=None):
    ...
    if isinstance(xtest, type(None)):
        ...
        for time_idx in range(sum(times <= t_max)):
            ...
            for feature_idx in range(len(features)):
                plt.plot(sizes[:xmax_idx], n_evo[time_idx, feature_idx, :xmax_idx],
                         color=colors[feature_idx],
                         label=write_feature_lengend(features[feature_idx]))
            ...
            sns.despine()
            if is_saved:
                path = write_fig_path(fig_dir, f'{time_idx:03d}.png',
                                      is_video=True, is_printed=False)
        ...
        for time_idx in range(time_count):
            path = write_fig_path(fig_subfolder, f'{time_idx:03d}.png',
                                  is_video=True, is_printed=False)
            ...
            if os.path.exists(path):
                ...
                fig, axes = plt.subplots(2, 1, sharex=False, sharey=False,
                                         figsize=(W, H), gridspec_kw=
                                         {'height_ratios': [1.8, 1]})
                axes[0].set_ylim(0, n_max)
                axes[1].set_ylim(0, 1.1 * ntest_max)
                axes[1].set_xlim(0, t_max)
                for feature_idx in range(len(features)):
                    axes[0].plot(sizes[:xmax_idx],
                                 n_evo[time_idx, feature_idx, :xmax_idx],
                                 color=colors[feature_idx], label=
                                 write_feature_lengend(features[feature_idx]),
                                 linewidth=2)
                    axes[1].plot(times_to_plot, ntest_tmp[:, feature_idx],
                                 color=colors[feature_idx], linewidth=2)
                ...
                axes[1].set_ylabel(LABELS['y_density'] + \
                                   rf' at $x={xtest:.2f}$', labelpad=8)
...
def plot_evo_distribution_discrete(fig_count, times, sizes, features, n_evo,
                                   x_max=None, name='', fig_dir=None,
                                   time_idxs=None, t_max=None,
                                   is_legend=True, figsize=None):
    ...
    for i in range(0, fig_count): # Iteration on subplots / times to plot.
        ...
        if is_legend and i == 0:
            ...
            axes[i].text(.96, .94, LEGENDS[i], transform=axes[i].transAxes,
                         va='top', ha='right')
...
def plot_evo_distribution_discrete_CFL1(fig_count, times, sizes, features,
                                        n_evo, x_max=None, name='',
                                        fig_dir=None):
    """ Plot in one unique figure, the size densities (up to the size of index
    'xmax_idx' and for all features) at 'fig_count' different times of 'times'.
    For appropriate legend 'is_mixing' is True if data come from a mixed
    ...
    for i in range(0, fig_count): # Iteration on subplots / times to plot.
        for feature_idx in range(feature_count): # A line per feature.
            axes[i].plot(sizes[i, :xmax_idx],
                         n_evo[time_idxs[i], feature_idx, :xmax_idx],
                         LINESTYLES[feature_idx], color= COLORS[feature_idx],
                         label=write_feature_lengend(features[feature_idx]))
        axes[i].text(.96, .94, LEGENDS[i], transform=axes[i].transAxes,
                     va='top', ha='right')
...
def plot_evo_distribution_at_fixed_size(times, features, n_along_fixed_x,
                                        fixed_size, name='', fig_dir=None,
                                        t_max=None, leg_loc="lower right",
                                        title=None, is_wo_vmax_plotted=False,
                                        figsize=None):
    if isinstance(t_max, type(None)):
        tmax_idx = -1
        name_add = ''
    ...
    for feature_idx in range(len(features))[::-1]:
        plt.plot(times[:tmax_idx], n_along_fixed_x[:tmax_idx, feature_idx],
                 color=COLORS[feature_idx],
                 label=write_feature_lengend(features[feature_idx]))
    ...
    sns.despine()
    if not isinstance(fig_dir, type(None)):
        path = write_fig_path(fig_dir, f'evo_n_fixed_x{fixed_size:.2f}_' +
                              name + name_add + '.pdf')
        plt.savefig(path, bbox_inches='tight')
    plt.show()
