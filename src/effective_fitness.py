...
Note: Depending on the publication trait may be used equivalently to feature.
      Here we use trait.
      For arXiv compatible figure names, IS_ARXIV_FRIENDLY_FIG_NAMES must be set
      to True in `write_path.py`.

TO DO: change optional rng argument to mandatory.
...
LABELS = {
    "arithmetic": r"$m_A$",  # r"$\mathrm{arithmetic~mean}$",
...
LABELS_MEANS = (
    f"{LABELS['arithmetic']}, {LABELS['geometric']}, " + f"{LABELS['harmonic']}"
)
...
def format_ticks_3D(format_x="%.1f", format_y="%.1f", format_z="%.1f"):
    ...
    with warnings.catch_warnings():
        ...
        ax.set_xticklabels(
            ax.get_xticks(), va="center_baseline", horizontalalignment="center"
        )
        ax.xaxis.set_major_formatter(ticker.FormatStrFormatter(format_x))

        ax.set_yticklabels(
            ax.get_yticks(), verticalalignment="baseline", horizontalalignment="left"
        )
...
def format_ticks_2D(ax=None, format_x="%.1f", format_y="%.1f"):
    ...
    if isinstance(format_x, str):
        ...
        txts = [f"{float(txt.get_text()):{format_x}}" for txt in ax.get_xticklabels()]
        ax.set_xticklabels(txts)
    # y-axis.
    if isinstance(format_y, str):
        ax.yaxis.set_major_formatter(ticker.FormatStrFormatter(format_y))
    else:
        txts = [f"{float(txt.get_text()):{format_y}}" for txt in ax.get_yticklabels()]
...
def approx_int_direct_constant(kappa, traits, beta=1, is_gap_needed=False):
    ...
    matrix = (
        beta * (2 * np.transpose(kappa) - np.identity(trait_count)) @ np.diag(traits)
    )
...
def compute_means(values_s):
    ...
    """
    means = {
        "arithmetic": [st.mean(values) for values in values_s],
...
def compute_means_of_2(fixed_value, values):
    """Computes the arithmetic, geometrical and harmonic means between
...
def generate_numbers(
    mid_point, interval_length, number_count, trait_choice="uniform", rng=None
):
...
def generate_numbers_w_varying_std(
    mid_point, interval_lengths, number_count, trait_choice="uniform", rng=None
):
    ...
    on the `set_count` intervals of length the lengths given by
    ...
    interval_lengths : ndarray
        1D array (set_count,) with all the lengths of the intervals to
        generate.
    number_count : int
    ...
    rng = np.random.default_rng(rng)
    numbers_s = [
        generate_numbers(mid_point, interval_length, number_count, trait_choice, rng)
        for interval_length in interval_lengths
    ]
    return numbers_s


def generate_numbers_w_varying_count(
    mid_point, interval_length, number_counts, trait_choice="uniform", rng=None
):
    ...
    interval_length : float
        The length of the interval I where numbers are taken.
    ...
    numbers_s = [
        generate_numbers(
            mid_point, interval_length, number_count, trait_choice, rng=rng
        )
...
def generate_kappa(traits, kappa_choice, diag=None, rng=None):
    ...
    ----------
    traits : ndarray
        1D array (trait_count,) of the individual traits (ordered).
    kappa_choice : str
    ...
    -------
    kappa : ndarray
         2D array (trait_count, trait_count) a stochastic matrix.
    ...
    if kappa_choice == "diag_n_uniform" or kappa_choice == "diag_n_uniform_specific":
...
def write_kappa_choice(kappa_choice, diag=None, is_shorten=False):
    ...
    if kappa_choice == "diag_n_uniform":
        if isinstance(diag, type(None)):
                ...
                rf"${k_str}{{ij}} = "
                + r"\frac{1-\alpha}{M-1}, \; \, {{\scriptstyle i \neq j}}$"
            )
            if is_shorten:
                return end
            return rf"${k_str}{{ii}} = \alpha, \quad $" + end
        one_minus_diag = "0.1%f" % (1.0 - diag)
...
def plot_2D_v_eff_and_mean_wrt_traits_constant(
    ...
    traits : ndarray
        Ordered individual traits (v_i = traits[i], with tau_i(x) = v_i).
    beta : float
    ...
    if isinstance(kappas, list):
        ...
        figsize = (6.2, 4.0)  # Default (6.4, 4.8)
    else:
        kappa_s = [kappas]
        figsize = (6.8, 3.8)
    ...
    if is_mirror:
        ...
        lambdas_r_s = [
            compute_lambdas_2D_wrt_traits(kappa_r, fixed_trait, traits_r, beta)
            for kappa_r in kappa_s
        ]
        lambdas_s = [
            np.concatenate([lambdas_l_s[i], lambdas_r_s[i]])
            for i in range(len(lambdas_l_s))
        ]
        matrices_s = [
            np.concatenate(
                [
                    [kappa_1to2_s[i] - 0.5 * np.identity(2) for v in traits_l],
                    [kappa_s[i] - 0.5 * np.identity(2) for v in traits_r],
                ]
            )
            for i in range(len(lambdas_l_s))
        ]
        ...
        lambdas_s = [
            compute_lambdas_2D_wrt_traits(kappa, fixed_trait, traits, beta)
            for kappa in kappa_s
        ]
        matrices_s = [
            [kappa - 0.5 * np.identity(2) for v in traits] for kappa in kappa_s
        ]
        ...
        lambdas_approx = [
            max(
                np.linalg.eig(
                    ...
                    * beta
                    * matrices_s[idx][i]
                    * np.array([[fixed_trait], [traits[i]]])
                )[0]
            )
            for i in range(len(traits))
        ]
        print(
            "max(|lambdas - lambdas_approx|): ",
            max(abs(lambdas_s[idx] - lambdas_approx)),
        )
        if is_error_plotted:
            plt.plot(
                lambdas_s[idx] - lambdas_approx, label=r"$\lambda_{th}-\lambda_{app}$"
            )
            plt.text(
                0.7, 0.15, write_traits(r"\{{ v_1, v_2 \}}"), transform=ax.transAxes
            )
        plt.plot(
            traits,
            lambdas_s[idx] / beta,
    ...
    leg1 = plt.legend(
        handles=plt.gca().get_lines()[: len(kappa_s)],
    ...
    plt.legend(
        handles=plt.gca().get_lines()[-3:], ncol=3, loc="lower right", columnspacing=0.6
    )
...
def plot_v_eff_and_means_w_varying_count_constant(
    ...
    kappas_s = [
        [generate_kappa(v, kappa_choices[i], diag=diags[i], rng=rng) for v in traits_s]
        for i in range(curve_count)
    ]
    lambdas_s = [
        np.array(
            [
                approx_lambda_constant(kappas[i], traits_s[i], beta)
                for i in range(len(traits_s))
            ]
        )
        for kappas in kappas_s
    ]
    ...
    for idx in range(curve_count):
        ...
        if len(kappa_choices) == 1:
            plt.title(write_kappa_choice(kappa_choice, diags[idx]), pad=8, loc="right")
    ...
    # Legend for kappa (1 column).
    leg1 = plt.legend(
        handles=plt.gca().get_lines()[:curve_count], bbox_to_anchor=bbox_to_anchor
    )
    plt.gca().add_artist(leg1)
    # Legend for means (3 columns).
    plt.legend(
...
def plot_v_eff_and_means_w_varying_std_constant(
    ...
    curve_count = len(kappa_choices)
    diags = [None] * curve_count
    idx_diag = [i for i, kc in enumerate(kappa_choices) if kc == "diag_n_uniform"]
    if isinstance(diag, list):
    ...
    traits_s = generate_numbers_w_varying_std(
        fixed_trait, interval_lengths, trait_count, trait_choice, rng=rng
    )
    kappas_s = [
        [
            generate_kappa(traits, kappa_choices[i], diag=diags[i], rng=rng)
    ...
    for idx in range(curve_count):
        ...
        plt.plot(
            ...
            color=PALETTE[idx],
            alpha=alpha,
            linewidth=factor * default_lwidth,
            zorder=zorders[idx],
        )
        if len(kappa_choices) == 1:
            plt.title(write_kappa_choice(kappa_choice, diags[idx]), pad=8, loc="right")
    ...
    plt.legend(bbox_to_anchor=bbox_to_anchor)  # fontsize="small",
...
def plot_v_eff_and_means_w_varying_std_n_correlation(
    ...
    par_beta,
    fig_directory,
    conserved_mean="arithmetic",
    is_classic_mean=False,
    is_title=True,
    is_small_only=False,
    case_coef="constant",
    formats=["small"],
    ...
    if conserved_mean == "arithmetic":
        ...
        traits_s = np.exp(
            generate_numbers_w_varying_std(
                np.log(fixed_trait), interval_lengths_geo, trait_count, "uniform"
            )
        )
    elif conserved_mean == "harmonic":
        csts = np.sqrt(interval_lengths**2 + fixed_trait**2)
        v1s = (-interval_lengths + fixed_trait + csts) / 2
        interval_lengths_har = interval_lengths / (v1s * (interval_lengths + v1s))
        traits_s = generate_numbers_w_varying_std(
    ...
    if "large" in formats:
        ...
        for i in range(curve_count):
            diag = diags[i]
            kappas = [
                generate_kappa(traits, "diag_n_uniform", diag=diag)
                for traits in traits_s
            ]
            if case_coef == "constant":
                lambdas = np.array(
                    [
                        approx_lambda_constant(kappas[i], traits_s[i], beta_constant)
                        for i in range(len(traits_s))
                    ]
                )
            else:
        ...
        if is_classic_mean:
            ...
            for key, means in means_s.items():
                if key == "arithmetic":
                    ...
                    plt.plot(interval_lengths, means, color="red", linestyle="--")
        plt.legend(
            title=r"$\alpha$",
    ...
    if "small" in formats:
        ...
        if is_title:
            plt.title(write_kappa_choice("diag_n_uniform"), pad=10, loc="right")
        else:
            to_add = to_add + "_no-title"
        if is_classic_mean:
            to_add = to_add + "_w-means"
        ...
        plt.legend(
            ...
            loc="upper left",
            edgecolor="#F0F0F0",
            prop={"size": 0.9 * sns.plotting_context()["legend.fontsize"]},
        )
...
def plot_my_surface(
    x, y, z, ticks=None, ticks_labels=None, label=None, angle_rotation=0
):
...
def compute_n_plot_heatmap_veff_wrt_kappa_constant(
    ...
    heatmap = {
        "lambda": np.zeros((coef_count, coef_count)),
        "mass": np.zeros((coef_count, coef_count)),
    }
    for i in range(coef_count):
        for j in range(coef_count):
            kappa = ker.kappa_bimodal(
                kappa_11=1 - kappa_coefficients[j], kappa_22=1 - kappa_coefficients[i]
            )
    ...
    df_lambda = pd.DataFrame(
        data=heatmap["lambda"], columns=kappa_coefficients, index=kappa_coefficients
    )
    plt.figure()
    tick_positions = np.linspace(0, 1, 11)  # Desired ticks
    tick_idxs = [
        np.argmin(np.abs(kappa_coefficients - tick)) for tick in tick_positions
    ]
    s = sns.heatmap(
        df_lambda,
        cmap=palette,
        xticklabels=tick_idxs,
        yticklabels=tick_idxs,
        cbar_kws=dict(format="{x:.2f}"),
        square=True,
        linewidths=0.0,
        rasterized=True,
    )
...
def compute_n_plot_distribution_bimodal_wrt_kappa(
    ...
    par_beta,
    kappa_varying_coefs,
    kappa_fixed_coef=None,
    par_ninit=PAR_N_INIT_LONGTIME,
    par_grids=PAR_GRIDS_CONSTANT_LONGTIME,
    fig_dir=None,
    is_no_heredity=False,
    xmax=None,
    fig_format="pdf",
):
    ...
    for cc in kappa_varying_coefs[::None]:
        ...
        if len(z) == 0:  # Legend and x-axis.
            ...
            z.append([out["irr"][1]])
            ax.plot(sizes, ys=z[-1][0][:idx_max], zs=coef, zdir="y", color="blue")
            z.append([out["irr"][0]])
            ax.plot(sizes, ys=z[-1][0][:idx_max], zs=coef, zdir="y", color="orange")
