...
Thus, if you need to define a new kappa, the best practise to avoid complicated
...
def kappa_partial_mixing(probability, is_slowest_mutating,
    ...
    if is_slowest_mutating:
        return np.array([[probability, 1-probability], [0, 1]]), \
            f'{name}-stf{proba_name}'  # , TMP_STF + rf"$( {probability} )$"
    else:
        return np.array([[1, 0], [1-probability, probability]]), \
            f'{name}-fts{proba_name}'  # , TMP_FTS + rf"$( {probability} )$"
...
def kappa_bimodal(kappa_11, kappa_22):
    ...
    return np.array([[kappa_11, 1 - kappa_11], [1 - kappa_22, kappa_22]]), \
        name


def compute_threshold_p0(features):
    """ Compute the (conjectured) threshold probabilty p_0 s.t. when kappa is
...
# Irreducible kernels.
KAPPAS_IRR = dict()
KAPPAS_IRR['3x3'] = (np.array([[0.7, 0.2, 0.1],
                               [0.5, 0.4, 0.1],
                               [0.3, 0.3, 0.4]]), 'irr', r"$\kappa^{irr}$")
KAPPAS_IRR['2x2'] = (np.array([[0.7, 0.4],
                               [0.5, 0.5]]), 'irr',  r"$\kappa^{irr}$")
