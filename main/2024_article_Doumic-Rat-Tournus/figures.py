...
out = fv.compute_longtime_approximation_2D_mix_vs_irr(
    IS_CONSERVATIVE, TRAITS, PAR_BETA_CONSTANT, KAPPA, is_normalized_by_v=False
)
...
#     xmax=X_MAX, palette=PALETTE, fig_format=FIG_FORMAT)
...
for traits in TRAITS_S:
    ...
    for i in range(len(KAPPA_COEFS)):
        ...
        z[i] = np.array(
            [
                fv.compute_lambda_2D(kappa, traits, PAR_BETA_CONSTANT[1])
                for kappa in kappas
            ]
        )
    fv.plot_my_surface(
...
LABELS = [fv.write_kappa_choice(kchoice, is_shorten=True) for kchoice in KAPPA_CHOICES]
...
LABELS = [
    fv.write_kappa_choice(KAPPA_CHOICES[i], diag=DIAGS[i], is_shorten=True)
    for i in range(2)
]
LABELS.extend(
    [fv.write_kappa_choice(kchoice, is_shorten=True) for kchoice in KAPPA_CHOICES[2:]]
)
